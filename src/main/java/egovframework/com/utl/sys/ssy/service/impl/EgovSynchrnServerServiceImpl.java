 ...
 *   2025.09.06  이선규          2025년 컨트리뷰션 PMD로 소프트웨어 보안약점 진단하고 제거하기-LocalVariableNamingConventions(final이 아닌 변수는 밑줄을 포함할 수 없음)
 *   2025.09.06  이선규          2025년 컨트리뷰션 PMD로 소프트웨어 보안약점 진단하고 제거하기-CloseResource(부적절한 자원 해제)
 *   2025.09.06  이선규          2025년 컨트리뷰션 PMD로 소프트웨어 보안약점 진단하고 제거하기-AssignmentInOperand(피연산자내에 할당문이 사용됨. 해당 코드를 복잡하고 가독성이 떨어지게 만듬)
 *
 *      </pre>
 */
...
public class EgovSynchrnServerServiceImpl extends EgovAbstractServiceImpl implements EgovSynchrnServerService {
	...
	public boolean processSynchrn(SynchrnServerVO synchrnServerVO, File[] uploadFile) throws Exception {
		...
		while (iterator.hasNext()) {
			SynchrnServerVO synchrnServerVO2 = iterator.next();

			reflctAt = processFtp(synchrnServerVO2.getServerIp(), Integer.parseInt(synchrnServerVO2.getServerPort()),
					synchrnServerVO2.getFtpId(), synchrnServerVO2.getFtpPassword(), synchrnServerVO2.getSynchrnLc(),
					synchrnServerVO.getFilePath(), uploadFile);

			synchrnServer.setServerId(synchrnServerVO2.getServerId());
			if (reflctAt) {
