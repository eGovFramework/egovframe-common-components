...
<body>
        <!-- content start -->
        <form:form id="umtMberPasswordForm" modelAttribute="mberPasswordManageVO" method="post" autocomplete="off" action="${pageContext.request.contextPath}/uss/umt/EgovMberPasswordUpdt.do" onsubmit="fnUpdate(document.forms[0]); return false;">
              <!-- onsubmit="javascript:return FormValidation(document.mberManageVO);" >  -->
...
<div class="wTableFrm">
	...
	<tbody>
		...
		<tr>
			<th>${title}</th>
			<td class="left">
                    <input name="mberId" id="mberId" type="text" size="20" value="<c:out value='${mberPasswordManageVO.mberId}'/>"  maxlength="20" readonly>
                    <input name="uniqId" id="uniqId" type="hidden" size="20" value="<c:out value='${mberPasswordManageVO.uniqId}'/>">
                    <input name="userTy" id="userTy" type="hidden" size="20" value="<c:out value='${mberPasswordManageVO.userTy}'/>">
			</td>
		</tr>
	...
	<button type="button" class="btn_s2" onClick="fnUmtPasswordCancel('umtMberPasswordForm'); return false;" title="<spring:message code="button.reset" /> <spring:message code="input.button" />"><spring:message code="button.reset" /></button>
