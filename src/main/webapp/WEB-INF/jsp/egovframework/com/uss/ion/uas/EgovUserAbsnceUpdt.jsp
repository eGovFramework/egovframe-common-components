 ...
 * 목록 으로 가기
 ******************************************************** */
function fn_egov_inqire_userAbsncelist() {
	userAbsnceVO.action = "<c:url value='/uss/ion/uas/selectUserAbsnceList.do'/>";
	userAbsnceVO.submit();
}
</script>
</head>
...
<div class="wTableFrm">
<form:form modelAttribute="userAbsnceVO" action="${pageContext.request.contextPath}/uss/ion/uas/updtUserAbsnce.do" method="post" onSubmit="fn_egov_updt_userAbsnce(document.forms[0]); return false;">
	<!-- 타이틀 -->
	...
	<table class="wTable">
		...
		<!-- 사용자ID -->
		<c:set var="title"><spring:message code="ussIonUas.userAbsnceUpdt.userId"/></c:set>
		<tr>
		...
		<!-- 사용자명 -->
		<c:set var="title"><spring:message code="ussIonUas.userAbsnceUpdt.userNm"/></c:set>
		<tr>
		...
		<!-- 부재여부 -->
		<c:set var="title"><spring:message code="ussIonUas.userAbsnceUpdt.userAbsnceAt"/></c:set>
		<tr>
		...
		<!-- 등록일시 -->
		<c:set var="title"><spring:message code="ussIonUas.userAbsnceUpdt.frstRegisterPnttm"/></c:set>
		<tr>
