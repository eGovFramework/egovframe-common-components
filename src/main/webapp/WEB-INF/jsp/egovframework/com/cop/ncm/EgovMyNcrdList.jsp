...
<body>
	<form name="frm" method="post" action="<c:url value='/cop/ncm/selectMyNcrdUseInf.do'/>">
		...
		<div class="board">
			<h1><spring:message code="comCopNcm.myNcrdList.title" /></h1>
			<!-- 나의 명함목록 -->
				...
				<ul>
					<li>
						<select name="searchCnd" title="<spring:message code="title.searchCondition" /><spring:message code="input.cSelect" />"><!-- 검색조건선택 -->
							<option value="0" <c:if test="${searchVO.searchCnd == '0'}">selected="selected"</c:if>>
							<spring:message code="cop.ncrdNm" />
							</option><!-- 이름 -->
							<option value="1" <c:if test="${searchVO.searchCnd == '1'}">selected="selected"</c:if>>
							<spring:message code="cop.cmpnyNm" />
							</option><!-- 회사명 -->
							<option value="2" <c:if test="${searchVO.searchCnd == '2'}">selected="selected"</c:if>>
			...
			<table class="board_list">
				<caption>
					<spring:message code="comCopNcm.myNcrdList.title" />
				</caption>
				<!-- 나의 명함목록 -->
				<colgroup>
					<col style="width:8%" />
				...
				<thead>
					<tr>
						...
						<th scope="col">
							<spring:message code="cop.ncrdNm" /> <!-- 이름 -->
						</th>
						<th scope="col">
							<spring:message code="cop.cmpnyNm" /> <!-- 회사명 -->
						</th>
						<th scope="col">
							<spring:message code="cop.deptNm" /> <!-- 부서명 -->
						</th>
						<th scope="col">
							<spring:message code="table.regdate" /> <!-- 등록일자 -->
						</th>
					</tr>
				</thead>
				<tbody>
