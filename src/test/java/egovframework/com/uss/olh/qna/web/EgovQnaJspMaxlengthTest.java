...
/**
 * Q&amp;A 등록·수정 화면의 JSP maxlength가 QnaVO의 @Size 제약을 넘지 않는지 확인한다.
 */
class EgovQnaJspMaxlengthTest {

	private static final String BASE = "src/main/webapp/WEB-INF/jsp/egovframework/com/uss/olh/qna/";

	private static final String[] FIELDS = { "wrterNm", "areaNo", "middleTelno", "endTelno", "emailAdres" };

	private static final Pattern HTML_COMMENT = Pattern.compile("<!--.*?-->", Pattern.DOTALL);

	@Test
	void qnaRegistMaxlengthWithinSize() throws Exception {
		assertMaxlengthWithinSize("EgovQnaRegist.jsp");
	}

	@Test
	void qnaUpdtMaxlengthWithinSize() throws Exception {
		assertMaxlengthWithinSize("EgovQnaUpdt.jsp");
	}
	...
	private void assertMaxlengthWithinSize(String jsp) throws Exception {
		...
		for (String field : FIELDS) {
			...
			assertTrue(jspMax <= sizeMax, jsp + " 의 " + field + " maxlength=" + jspMax
					+ " 가 QnaVO 의 @Size(max=" + sizeMax + ") 를 넘습니다.");
		}
	}

	private int sizeMaxOf(String field) throws NoSuchFieldException {
		Field f = QnaVO.class.getDeclaredField(field);
