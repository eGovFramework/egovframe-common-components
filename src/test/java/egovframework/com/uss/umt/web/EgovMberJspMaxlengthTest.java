...
/**
 * 회원관리 화면의 JSP maxlength가 MberManageVO의 @Size 제약을 넘지 않는지 확인한다.
 */
class EgovMberJspMaxlengthTest {

	private static final String BASE = "src/main/webapp/WEB-INF/jsp/egovframework/com/uss/umt/";

	private static final String[] FIELDS = { "mberNm", "areaNo", "middleTelno", "endTelno" };

	private static final Pattern HTML_COMMENT = Pattern.compile("<!--.*?-->", Pattern.DOTALL);

	@Test
	void mberSelectUpdtMaxlengthWithinSize() throws Exception {
		assertMaxlengthWithinSize("EgovMberSelectUpdt.jsp");
	}
	...
	private void assertMaxlengthWithinSize(String jsp) throws Exception {
		...
		for (String field : FIELDS) {
			...
			assertTrue(jspMax <= sizeMax, jsp + " 의 " + field + " maxlength=" + jspMax
					+ " 가 MberManageVO 의 @Size(max=" + sizeMax + ") 를 넘습니다.");
		}
	}

	private int sizeMaxOf(String field) throws NoSuchFieldException {
		Field f = MberManageVO.class.getDeclaredField(field);
