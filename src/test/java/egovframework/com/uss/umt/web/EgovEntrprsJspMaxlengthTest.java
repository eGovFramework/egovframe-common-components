...
/**
 * 기업사용자관리 화면의 JSP maxlength가 EntrprsManageVO의 @Size 제약을 넘지 않는지 확인한다.
 */
class EgovEntrprsJspMaxlengthTest {

	private static final String BASE = "src/main/webapp/WEB-INF/jsp/egovframework/com/uss/umt/";

	private static final String[] FIELDS = { "cmpnyNm", "areaNo", "entrprsMiddleTelno", "entrprsEndTelno" };

	private static final Pattern HTML_COMMENT = Pattern.compile("<!--.*?-->", Pattern.DOTALL);

	@Test
	void entrprsInsertMaxlengthWithinSize() throws Exception {
		assertMaxlengthWithinSize("EgovEntrprsInsert.jsp");
	}

	@Test
	void entrprsSbscrbMaxlengthWithinSize() throws Exception {
		assertMaxlengthWithinSize("EgovEntrprsSbscrb.jsp");
	}

	@Test
	void entrprsSelectUpdtMaxlengthWithinSize() throws Exception {
		assertMaxlengthWithinSize("EgovEntrprsSelectUpdt.jsp");
	}
	...
	private void assertMaxlengthWithinSize(String jsp) throws Exception {
		...
		for (String field : FIELDS) {
			...
			assertTrue(jspMax <= sizeMax, jsp + " 의 " + field + " maxlength=" + jspMax
					+ " 가 EntrprsManageVO 의 @Size(max=" + sizeMax + ") 를 넘습니다.");
		}
	}

	private int sizeMaxOf(String field) throws NoSuchFieldException {
		Field f = EntrprsManageVO.class.getDeclaredField(field);
