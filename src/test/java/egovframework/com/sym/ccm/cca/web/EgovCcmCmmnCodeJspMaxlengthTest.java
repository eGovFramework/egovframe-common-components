...
/**
 * 공통코드 화면의 클라이언트측 입력 제한이 CmmnCode의 @Size 제약을 넘지 않는지 확인한다.
 * <p>
 * 화면이 바인딩하는 cmmnCodeVO(CmmnCodeVO)는 CmmnCode를 상속하므로 제약의 출처는 CmmnCode다.
 * JSP의 maxlength 속성과 EgovValidation.js의 검증 규칙 두 곳 모두 확인한다.
 * </p>
 */
class EgovCcmCmmnCodeJspMaxlengthTest {

	private static final String BASE = "src/main/webapp/WEB-INF/jsp/egovframework/com/sym/ccm/cca/";

	private static final String VALIDATION_JS = "src/main/webapp/js/egovframework/com/cmm/EgovValidation.js";

	private static final String[] FIELDS = { "codeId", "codeIdNm" };

	private static final Pattern HTML_COMMENT = Pattern.compile("<!--.*?-->", Pattern.DOTALL);

	@Test
	void cmmnCodeRegistMaxlengthWithinSize() throws Exception {
		assertMaxlengthWithinSize("EgovCcmCmmnCodeRegist.jsp");
	}

	@Test
	void cmmnCodeUpdtMaxlengthWithinSize() throws Exception {
		assertMaxlengthWithinSize("EgovCcmCmmnCodeUpdt.jsp");
	}
	...
	void validationJsRuleWithinSize() throws Exception {
		...
		String block = validateCmmnCodeVoBlock(source);

		for (String field : FIELDS) {
			int sizeMax = sizeMaxOf(field);
			Matcher m = Pattern.compile(field + "\\s*:\\s*\\{.*?maxlength\\s*:\\s*(\\d+)", Pattern.DOTALL).matcher(block);
			assertTrue(m.find(), "validateCmmnCodeVO 에서 " + field + " 의 maxlength 규칙을 찾지 못했습니다.");
			int jsMax = Integer.parseInt(m.group(1));
			assertTrue(jsMax <= sizeMax, "EgovValidation.js 의 " + field + " maxlength=" + jsMax
					+ " 가 CmmnCode 의 @Size(max=" + sizeMax + ") 를 넘습니다.");
		}
	}
	...
	/** validateCmmnCodeVO 함수 본문만 잘라낸다. 같은 필드명이 다른 함수에도 있으므로 범위를 좁힌다. */
	private String validateCmmnCodeVoBlock(String source) {
		int start = source.indexOf("function validateCmmnCodeVO");
		assertTrue(start >= 0, "EgovValidation.js 에서 validateCmmnCodeVO 를 찾지 못했습니다.");
	...
	private int sizeMaxOf(String field) throws NoSuchFieldException {
		Field f = CmmnCode.class.getDeclaredField(field);
